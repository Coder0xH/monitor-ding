...
本模块包含期货订单的创建、查询、取消等操作的API路由。
支持市价单、限价单、分批订单等多种订单类型。
...
from app.core.models import FuturesOrderRequest
...
@router.get("/{symbol}")
async def get_open_orders(symbol: str, api_key_id: str = None):
    """
    获取指定交易对的开放订单
    ...
    Raises:
        HTTPException: 如果币安API不可用或获取订单失败
    """
    # 获取交易所实例
    ...
    try:
        ...
        logger.info(f"获取到 {symbol} 的 {len(orders)} 个开放订单")
        return {
            "status": "success",
            "message": f"获取到 {symbol} 的 {len(orders)} 个开放订单",
            "orders": orders
        }
        
    except Exception as e:
        logger.error(f"获取订单错误: {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def cancel_order(order_id: str, symbol: str, api_key_id: str = None):
    """
    取消指定订单
    ...
    Raises:
        HTTPException: 如果币安API不可用或订单取消失败
    """
    # 获取交易所实例
    exchange = get_exchange_instance(api_key_id)
    if not exchange:
...
async def get_batch_order_status(batch_id: str):
    ...
    return {
        ...
        "batch_info": {
            "batch_id": batch_id,
            "symbol": batch_info['symbol'],
            "side": batch_info['side'],
            "total_amount": batch_info['total_amount'],
            "executed_amount": batch_info['executed_amount'],
            "progress_percentage": (batch_info['executed_amount'] / batch_info['total_amount']) * 100,
            "orders_count": len(batch_info['orders']),
